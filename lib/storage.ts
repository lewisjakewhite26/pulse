...
// In-memory only: unlock survives tab navigation within a page load, resets on refresh.
...
export function getProfile(): PulseProfile | null {
  return read<PulseProfile | null>(KEYS.profile, null);
}
...
function targetOrDefault(value: string | undefined, fallback: number): number {
...
export function computeWellnessScore(
  ...
  const calorieTarget = targetOrDefault(profile?.calorieTarget, 2000);
  const proteinTarget = targetOrDefault(profile?.proteinTarget, 120);
  const waterTarget = targetOrDefault(profile?.waterTarget, 2500);
  const stepsTarget = targetOrDefault(profile?.stepsTarget, 8000);
  ...
  const alcoholLimit = profile?.alcoholUnits
    ? parseFloat(profile.alcoholUnits) || 14
    : 14;
